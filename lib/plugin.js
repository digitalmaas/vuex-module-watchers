...
export default function vuexWatcherPlugin (store) {
  if (store._watchersTracker) {
    for (const entry of store._watchersTracker.getPendingWatchers()) {
      const unsubscribers = initialiseWatcher(store, entry)
      store._watchersTracker.setUnsubscribers(entry.namespace, unsubscribers)
    }
  } else {
